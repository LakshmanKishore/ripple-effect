...
const localIceCandidatesTextarea = document.getElementById('local-ice-candidates');

const remoteIceCandidatesTextarea = document.getElementById('remote-ice-candidates');
const addRemoteIceCandidatesBtn = document.getElementById('add-remote-ice-candidates');
...
function setupDataChannel() {
    dataChannel.onopen = () => {
        ...
        connectionStatus.textContent = 'Connection established! You can now send messages.';
    };
...
createOfferBtn.onclick = async () => {
    ...
    connectionStatus.textContent = 'Offer created. Copy SDP and local ICE candidates to other peer.';
};
...
receiveOfferAndCreateAnswerBtn.onclick = async () => {
    ...
    connectionStatus.textContent = 'Answer created. Copy SDP and local ICE candidates to other peer.';
};
...
addRemoteIceCandidatesBtn.onclick = async () => {
    ...
    try {
        ...
        candidatesToProcess = inputValue.split('\n').filter(line => line.trim() !== '').map(line => JSON.parse(line));
    }
...
advertiseBtn.onclick = async () => {
    try {
        ...
        const sdpOfferCharacteristic = await service.getCharacteristic(SDP_OFFER_UUID);
        const iceCandidatesCharacteristic = await service.getCharacteristic(ICE_CANDIDATES_UUID);

        await sdpOfferCharacteristic.writeValue(new TextEncoder().encode(sdpOffer));
        await iceCandidatesCharacteristic.writeValue(new TextEncoder().encode(iceCandidates));
...
scanBtn.onclick = async () => {
    try {
        ...
        const service = await gattServer.getPrimaryService(SERVICE_UUID);
        const sdpAnswerCharacteristic = await service.getCharacteristic(SDP_ANSWER_UUID);
        const iceCandidatesCharacteristic = await service.getCharacteristic(ICE_CANDIDATES_UUID);
        ...
        remoteIceCandidatesTextarea.value = new TextDecoder().decode(iceCandidatesValue);
